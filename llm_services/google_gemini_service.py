...
class GoogleGeminiService(BaseLLM):
    ...
    def get_all_models(self):
        """모든 사용 가능한 모델을 반환 (최신 버전이 아닌 모델 포함)"""
        if not self.api_key: 
            print("Google Gemini API 키가 설정되지 않았습니다.")
            return []
        try:
            ...
            print(f"Google Gemini 전체 모델 목록 가져오기 실패: {e}")
            return []
    ...
    def translate(self, text, target_language, model_name):
        ...
        try:
            if not model_name.startswith('models/'):
                if "1.5" in model_name or "experimental" in model_name or model_name.endswith("-latest") or "1.0-pro" in model_name:
